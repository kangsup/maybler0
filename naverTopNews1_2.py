...
for newstype_index, newstype in enumerate(newstypes):
	...
	newstype_list = dom.select("#ranking_%s > ul > li" %newstype_index)
	print('\n'+'*'*50)
	print(newstype)
	print('*'*50)
	for j, newsItem in enumerate(newstype_list):
		newsTitleAnchor = newsItem.select("a")[0]
		print(j+1, "위: ", newsTitleAnchor.attrs.get('title'), "\n\t주소: ", newsTitleAnchor.attrs.get('href'))
	print('\n')

print("-"*60)

nowtime = dom.select('#today_main_news > div.com_header > span > em')
print('현재 시각', nowtime[0].text, '\n')